...
import FilesCardSkeleton from "./FilesSkeleton";
...
const Home = () => {
    ...
    const { files, isDeleting, loading, fetched } = useSelector((state) => {
        console.log('Files in Component:', state.files.files);
        return state.files;
    });
    const navigate = useNavigate();
    const allFilesContainerRef = useRef();

    // Search feature
    const filteredFiles = useMemo(() => {
        console.log('Recomputing filteredFiles:', files);
    ...
    useEffect(() => {
        ...
        if (allFilesContainerRef.current) {
            allFilesContainerRef.current.addEventListener('touchstart', holdToStartDeleteMode, { passive: true });
        }
    ...
    // Truncate file names that are too longer
    const truncateFileName = (fileName, maxChars) => {
        const fileExtension = fileName.match(/\.\w+$/)?.[0] || ''; // Extract file extension
        const baseName = fileName.replace(/\.\w+$/, ''); // Remove the file extension from the name
        ...
        const visibleChars = maxChars - fileExtension.length - 3; // Space for "..."

        if (visibleChars <= 0) {
            return '...' + fileExtension; // If no space, show only the extension
        }

        return `${baseName.slice(0, visibleChars)}...${fileExtension}`; // Truncate and append extension
    }

    // Example usage:
    const longFileName = "hello_world_this_is_a_very_long_paragraph_filename.pdf";
    const shortFileName = truncateFileName(longFileName, 20);

    console.log(shortFileName); // Output: "hello_world_...pdf"
        ...
        <>
            ...
            {files.length ? (
                <div className="main-container">
                        ...
                        {deleteMode ? (
                            <label
                                ...
                                <span
                                    ...
                                    className="checkbox-custom-checkmark"></span>
                                Select all
                            </label>
                    ...
                    {/* grid for files */}
                    <div
                        ref={allFilesContainerRef}
                        className={`dashboard-all-uploaded-files`}>
                        {fetched ? (
                            filteredFiles.map(file => {
                                // Extract the file extension
                                const fileExtension = file.name.match(/\.\w+$/)?.[0] || '';
                                // Get the base name without the extension
                                const baseName = file.name.replace(/\.\w+$/, '');
                                // Truncate the base name dynamically if it's too long
                                const maxLength = 20; // Set your desired maximum length
                                const truncatedName = baseName.length > maxLength
                                    ? `${baseName.slice(0, maxLength - 3)}...${fileExtension}`
                                    : file.name;
                                ...
                                return (
                                    <label
                                        key={file.id}
                                        data-id={file.id}
                                        className={`book ${deleteMode ? 'unselected' : 'hover'}`}
                                    ...
                                    >
                                        ...
                                        <div className="file-title-and-info">
                                            ...
                                            <p className="font-small bold">{truncatedName}</p>
                                        </div>
                                        ...
                                        {deleteMode && (
                                            <>
                                                <input
                                                    type="checkbox"
                                                    className="custom-checkbox files"
                                                    value={file.id}
                                                    checked={deleteFiles.includes(file.id)}
                                                    onChange={() => handleSelectedFiles(file.id)}
                                                />
                                                <span className="checkbox-custom-checkmark files"></span>
                                            </>
                                        )}
                                    </label>
                                );
                            ...
                            <FilesCardSkeleton onFetched={fetched} />
                        )}
                    </div>
